 ...
 * receipts.js  v15
 * 
 * Full receipt system with:
 *  • Hotel logo (base64, stored in hotel.logoUrl) shown at top of every receipt
 *  • "Powered by Anchor Suites Limited" on every printed/emailed receipt
 *  • POS receipt  — GET /api/receipts/pos/:saleId
 *  • Room receipt — GET /api/receipts/room/:roomLogId
 *  • Email POS    — POST /api/receipts/pos/:saleId/email
 *  • Email Room   — POST /api/receipts/room/:roomLogId/email
 *  • Auto-email on checkout if guest has email on file
 */
...
const WHITE  = '#FFFFFF';
const RED    = '#DC2626';
const BRAND  = '#1A3A5C';  // Anchor Suites brand color
const RECEIPT_WIDTH = 400;

// ─────────────────────────────────────────────────────────────────────────────
// GET /api/receipts/pos/:saleId  — Download POS receipt PDF
// ─────────────────────────────────────────────────────────────────────────────
router.get('/pos/:saleId', authenticate, requireLevel(3), async (req, res) => {
  try {
    ...
    const doc = await buildPOSReceipt(sale);
    doc.pipe(res);
    doc.end();
  } catch (err) {
...
// ─────────────────────────────────────────────────────────────────────────────
// GET /api/receipts/room/:roomLogId  — Download room stay receipt PDF
// ─────────────────────────────────────────────────────────────────────────────
router.get('/room/:roomLogId', authenticate, requireLevel(5), async (req, res) => {
  try {
    ...
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `inline; filename="${filename}"`);
    const doc = await buildRoomReceipt(log);
    doc.pipe(res);
    doc.end();
  } catch (err) {
...
// ─────────────────────────────────────────────────────────────────────────────
// POST /api/receipts/pos/:saleId/email  — Email POS receipt
// ─────────────────────────────────────────────────────────────────────────────
router.post('/pos/:saleId/email', authenticate, requireLevel(3), async (req, res) => {
  try {
    ...
    const sale = await prisma.pOSSale.findUnique({
      where: { id: req.params.saleId },
      include: { items: true, hotel: true, staff: { select: { name: true } },
        roomLog: { select: { guestName: true, room: { select: { number: true } } } },
      },
    });
    if (!sale) return res.status(404).json({ error: 'Sale not found' });
    const result = await emailReceipt({
      email, hotel: sale.hotel,
      subject: `Your receipt from ${sale.hotel.name}`,
      pdfBuffer: await streamToBuffer(await buildPOSReceipt(sale)),
...
// ─────────────────────────────────────────────────────────────────────────────
// POST /api/receipts/room/:roomLogId/email  — Email room receipt
// ─────────────────────────────────────────────────────────────────────────────
router.post('/room/:roomLogId/email', authenticate, requireLevel(5), async (req, res) => {
  try {
    ...
    if (log.status === 'CHECKED_OUT' && log.checkedOutAt) {
      ...
      if (elapsed > WINDOW_MS && !isGM) {
        return res.status(403).json({
          error: 'Receipt window expired. Only a General Manager can resend.',
          gmRequired: true,
        });
      }
    }
    ...
    const result = await emailReceipt({
      email: recipientEmail, hotel: log.hotel,
      subject: `Your stay receipt from ${log.hotel.name}`,
      pdfBuffer: await streamToBuffer(await buildRoomReceipt(log)),
      filename: `Stay-Receipt-${log.guestName.replace(/\s+/g, '-')}-${log.id.slice(-8).toUpperCase()}.pdf`,
    });

    if (result.success) {
      await prisma.roomLog.update({ where: { id: log.id }, data: { receiptSentAt: new Date() } });
    }
...
async function buildPOSReceipt(sale) {
  ...
  const W = RECEIPT_WIDTH - 48;
  let y = 20;

  y = await drawHotelHeader(doc, hotel, W, y);
  ...
  const saleDate  = new Date(sale.createdAt).toLocaleString('en-NG', { dateStyle: 'medium', timeStyle: 'short' });
  ...
  sale.items.forEach((item) => {
    ...
    doc.fillColor(TEAL).fontSize(6.5).text(item.category || '', 24, y + 8, { width: 80 });
  ...
  y += 13;
  doc.text(`VAT (${Number(hotel.vatPercent ?? 7.5)}%)`, 24, y);
  ...
  doc.text(`Payment: ${sale.paymentType?.replace(/_/g, ' ') || 'Cash'}`, 24, y);
  ...
  doc.page.height = y + 90;
  return doc;
}

async function buildRoomReceipt(log) {
  const doc = new PDFDocument({
    size: 'A5',
  ...
  y = await drawHotelHeader(doc, hotel, W, y, 28);
  ...
  if (log.posSales && log.posSales.length > 0) {
    ...
    log.posSales.forEach((s) => {
      s.items.forEach((item) => {
        ...
        if (!posByCategory[cat]) posByCategory[cat] = 0;
        posByCategory[cat] += Number(item.totalPrice);
      });
    });
  ...
  twoCol(doc, 28, y, W, `VAT (${Number(log.hotel.vatPercent ?? 7.5)}%):`, fmt(vatAmount2), false, 9, DGREY); y += 14;
...
async function drawHotelHeader(doc, hotel, W, y, marginLeft = 24) {
  const pad = 10;
  const iW  = W - pad * 2;

  // ── Logo (if available) ───────────────────────────────────────────────────
  if (hotel.logoUrl && hotel.logoUrl.startsWith('data:image')) {
    try {
      const base64Data = hotel.logoUrl.split(',')[1];
      const imgBuffer  = Buffer.from(base64Data, 'base64');
      const logoH = 48;
      const logoW = Math.min(160, W);

      // Light grey logo band
      doc.rect(marginLeft, y, W, logoH + 12).fill(LGREY);
      doc.image(imgBuffer, marginLeft + pad, y + 6, {
        height: logoH,
        width:  logoW,
        fit:    [logoW, logoH],
      });
      ...
      // Logo failed to render — fall through to text header
    }
  }
  ...
  const detailLines = [];
  if (hotel.address) detailLines.push({ label: 'Address:', text: hotel.address });
  if (hotel.phone)   detailLines.push({ label: 'Phone:',   text: hotel.phone });
  if (hotel.email)   detailLines.push({ label: 'Email:',   text: hotel.email });

  const bandH = Math.max(32, detailLines.length * 14 + 10);
  doc.rect(marginLeft, y, W, bandH).fill(LGREY);

  let dy = y + 7;
  detailLines.forEach(({ label, text }) => {
    doc.fontSize(7.5).font('Helvetica-Bold').fillColor(NAVY)
       .text(label, marginLeft + pad, dy, { width: 45 });
    doc.fontSize(7.5).font('Helvetica')
       .fillColor(label === 'Email:' ? TEAL : DGREY)
       .text(text, marginLeft + pad + 48, dy, { width: iW - 48 });
    dy += 14;
  });

  // Gold accent line
  doc.rect(marginLeft, y + bandH, W, 2.5).fill(GOLD);
  y += bandH + 6;
...
function drawReceiptFooter(doc, hotel, y, W, marginLeft = 24) {
  ...
  if (hotel.address) {
    doc.fontSize(7.5).font('Helvetica').fillColor(DGREY).text(hotel.address, marginLeft, y, cx);
    y += 11;
  }
  if (hotel.phone) {
    doc.fontSize(7.5).font('Helvetica').fillColor(DGREY).text('Tel: ' + hotel.phone, marginLeft, y, cx);
    y += 11;
  }
  if (hotel.email) {
    doc.fontSize(7.5).font('Helvetica').fillColor(TEAL).text(hotel.email, marginLeft, y, cx);
    y += 13;
  }

  // Divider
  doc.rect(marginLeft + W * 0.2, y, W * 0.6, 0.5).fill(MGREY);
  y += 8;

  // ── "Powered by Anchor Suites Limited" brand stamp ────────────────────────
  const stampW = 200;
  const stampX = marginLeft + (W - stampW) / 2;
  doc.rect(stampX, y, stampW, 18).fill(BRAND);
  doc.fontSize(7).font('Helvetica-Bold').fillColor(WHITE)
     .text('Powered by Anchor Suites Limited', stampX, y + 6, { width: stampW, align: 'center' });
  y += 22;

  // Tiny regulatory note
  doc.fontSize(6).font('Helvetica').fillColor(MGREY)
     .text('This receipt is computer-generated. No signature required.', marginLeft, y, cx);
}
...
async function emailReceipt({ email, hotel, subject, pdfBuffer, filename }) {
  ...
  try {
    ...
    const logoHtml = hotel.logoUrl && hotel.logoUrl.startsWith('data:image')
      ? `<img src="${hotel.logoUrl}" alt="${hotel.name}" style="max-height:60px;max-width:180px;margin-bottom:8px;">`
      : '';
        ...
        <div style="font-family:Arial,sans-serif;max-width:520px;margin:auto;border:1px solid #e2e8f0;border-radius:8px;overflow:hidden;">
          ...
          <div style="padding:20px 24px;">
            ...
            <p style="color:#94A3B8;font-size:0.75rem;text-align:center;">
              Powered by <strong>Anchor Suites Limited</strong> · Anchor Hotel Suite v15
            </p>
          </div>
        </div>`,
...
// ── Exported helper (used by roomLogs.js on auto-checkout) ────────────────────
